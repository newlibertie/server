...
class ApplicationErrorEnumTest extends WordSpec with Matchers with LazyLogging {
  ...
  class TestApplicationErrorEnum (errEnumName: String, initial:Int, capacity:Int) extends BaseError(errEnumName, initial, capacity)
  ...
  "the enum creation" should {
    "detect duplicate enum name" in {
      class TestApplicationErrorEnum2 (errEnumName: String, initial:Int, capacity:Int) extends BaseError(errEnumName, initial, capacity)
    ...
    "detect overlapping id range" in {
      var stat = -1
      class TestApplicationErrorEnum3 (errEnumName: String, initial:Int, capacity:Int) extends BaseError(errEnumName, initial, capacity)
    ...
    "detect duplicate id" in {
      var stat = -1
      class TestApplicationErrorEnum4 (errEnumName: String, initial:Int, capacity:Int) extends BaseError(errEnumName, initial, capacity)
      try {
